...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        for i, data in enumerate(train_data, start=epoch_iter):
            ...
            loss.backward()
            optimizer.step()
            train_bn_loss.update(loss.cpu().detach().numpy())
            #update metric
        #     tr_target = data['label'].detach()
        #     tr_pred = torch.argmax(cd_pred.detach(), dim=1)
        #
        #     tr_acc = running_metric.update_cm(pr=tr_pred.cpu().numpy(), gt=tr_target.cpu().numpy())
        #     if total_steps % opt.print_freq == print_delta:
        #         errors = {k: v.data.item() if not isinstance(v, int) else v for k, v in loss_dict.items()}
        #         t = (time.time() - iter_start_time) / opt.print_freq
        #         visualizer.print_current_errors_acc(opt.phase, epoch, epoch_iter, errors, tr_acc, t)
        #         visualizer.visualize_current_results(opt.phase, epoch, data['t1_img'].detach(), data['t2_img'].detach(), data['label'].detach(), tr_pred.unsqueeze(1))
        #
        # tr_scores = running_metric.get_scores()
        # visualizer.print_scores(opt.phase, epoch, tr_scores)
        # epoch_acc = tr_scores['mf1']
        # TRAIN_ACC = np.append(TRAIN_ACC, [epoch_acc])
        # np.save(os.path.join(opt.checkpoint_dir, opt.name, 'train_acc.npy'), TRAIN_ACC)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_data):
                ...
                val_pred = F.sigmoid(val_pred[2]).cpu().detach() > 0.5
                val_acc = running_metric.update_cm(pr=val_pred.cpu().numpy(), gt=val_target.cpu().numpy())
                # visualizer.visualize_current_results(opt.phase, epoch, data['t1_img'].detach(), data['t2_img'].detach(),
                #                                      data['label'].detach(), val_pred.unsqueeze(1))
        ...
        print('End of epoch %d / %d \t Time Taken: %d sec \t best acc: %.5f (at epoch: %d) \t loss: %.5f' %
            (epoch, opt.num_epochs + opt.num_decay_epochs, time.time() - epoch_start_time, best_val_acc, best_epoch, train_bn_loss.val))
